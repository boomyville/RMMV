 ...
 * 
 * @param Move Formula
 * @desc Default formula for choosing where to move if not target is available (SRPG_AIControl.js)
 * @default nearestOpponent
 * 
 * @param Max Distance
 * @desc Maximum range in steps which a movement will occur if no target in range. If no path found within X steps, then fallback movement occurs
 * @type number
 * @min 10
 * @default 100
 * 
 * @param Fallback Movement
 * @desc If no path can be found, default to MV pathfinding or not (if set to false, no movement occurs). This is an eval (so you could use $gameVariables.value(3) > 3 for example)
 * @type boolean
 * @on Yes
 * @off No
 * @default true
...
(function() {
    ...
    var _fallbackMovement = eval(parameters['Fallback Movement']);
        ...
        if (user.battleMode() === 'stand') {
            ...
            if ($gameTemp.isSrpgPriorityTarget() && !$gameTemp.isSrpgPriorityTarget().isErased()) {
                ...
                if (!priorityUnit || !priorityUnit[1].priorityTag('aiIgnore')) {
                    if ($gameTemp.isSrpgPriorityTarget().pathTo(x, y).length > 0) {
                        return -$gameTemp.isSrpgPriorityTarget().pathTo(x, y).length;
                    } else if (_fallbackMovement && !$gameTemp.isSrpgPriorityTarget().pathTo(x, y)) {
    ...
    Game_Character.prototype.pathTo = function(x, y) {
        ...
        if (x >= width || y >= height || x < 0 || y < 0) {
            ...
            return this.findShortestPath([x, y], grid);
        }
    };
})();
